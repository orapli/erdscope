...
- **既存データベースへの新メンバーオンボーディング。** 読み取り専用アカウントで稼働中の
  MySQL/PostgreSQL/SQLiteに接続すれば、実際にデプロイされているスキーマを探索できる図が
  得られます。手描きの古いER図や、そもそも図がない状態から解放されます。
- **データベースに接続できない状態でのモデルレビュー。** `--models` にRails、Prisma、
  Django、SQLAlchemy、Laravelいずれかのプロジェクトを渡せば、静的解析だけで同様の図を
  生成できます。DBの認証情報を持っていないコードベースのレビューに便利です。
- **顧客・監査向けのHTML/Excel納品。** 出力は自己完結した単一のHTMLファイル1つで、
  相手側に何かをインストール・ホストしてもらう必要はありません。`notes:` で設計意図を
  添えたり、スプレッドシートで欲しい相手向けにExcelワークブックを併せて出力できます。

## スキーマを預けても安心な理由

- **完全にローカルで動作し、渡したデータベース接続以外は外部へ何も送信しません。**
  テレメトリも外部サービスとの連携もありません。
- **読み取り専用のDBアカウントで十分です。** erdscopeはスキーマのメタデータを
  読み取るだけで、書き込みは一切行いません。
- **生成されるHTMLは自己完結型です。** CDNへの依存も、開いた後に発生する追加通信も
  ありません。
- DB構造・コード側の関連・独自ドキュメントの3つを1つのファイルに統合できる点が、
  いずれか1つしか扱わない他のツールとの違いです。
