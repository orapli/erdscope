...
The viewer and Excel exporter predate structured provenance. `serialize_for_viewer()` deep-copies the IR, removes `provenance`/`sources`, and emits only `manual`, `db_fk`, or `inferred` booleans; declared framework associations carry no flag. Tests explicitly prevent internal fields from leaking (`tests/test_pipeline.py`). Provider boundaries now validate this IR shape before merge: database adapters must return complete table fields, while framework overlays may return sparse association-only fragments; invalid third-party output fails with a provider-local path and kind/provider check (`src/erdscope/ir.py`, `providers.py`, `sources.py`).
...
`src/erdscope/db/base.py` defines `DBAdapter`, `register_adapter`, and the URL-scheme registry. Registration validates adapter metadata and later same-scheme registrations replace earlier ones, which makes trusted plugin reloads deterministic. Built-ins:
...
`src/erdscope/viewer.html` is a standalone SVG application with no runtime data fetch. It provides exploration, filtering, layout editing, persistent named views, and PNG/SVG/Mermaid/PlantUML exports. The Data Dictionary/Schema Grid is a standalone modal/tab with advanced filtering, table/column/relation views, inline and modal note editing, and links back to read-only table views. Viewer edits can add proposed ToBe tables/columns, persist proposed schema and notes across reloads, export re-importable Config JSON or a Download Updated HTML snapshot, and show an unsaved/unexported-changes guard. Persisted config is written only after an actual edit; an unmodified page load does not create an empty `persisted_config` entry in `localStorage`, and Discard clears the persisted changes before reload. Overview layout now has persisted Vertical, Horizontal, and viewport-scored Auto orientation policies; Focus remains vertical, and orientation is independent of relation traversal Direction. Straight-first relation routing uses direct segments when clear, bounded orthogonal detours around obstacles, and Bézier curves only as fallback. Recent layout behavior treats group frames as obstacles only for newly auto-placed tables, adds right/bottom multi-select alignment, lets auto-expanded neighbors become explicit selections, keeps depth-1 parents in the same vertical band as their deeper children when wrapped rows alternate, and retains auto-expanded neighbors as a distinct `KEPT` state when Auto-expand is turned off. Overview rows and nodes classify ROOT/AUTO/KEPT states; retained tables remain unchecked and non-root until promoted, removed, banned, or reset, while turning Auto-expand back on clears retention and recomputes the BFS. The retained set is persisted in `localStorage`, named views, and share payloads with tolerant handling of old or invalid values. `refreshView()` tracks the actually rendered display set separately from `nodePos`, whose coordinates intentionally outlive visibility, so 0-to-1 reappearance and full auto-tidy relayouts refit the viewport. State is otherwise stored in browser `localStorage`, namespaced by document title. `src/erdscope/exporters.py` writes XLSX directly with stdlib ZIP/XML and supports a five-style-cell template contract.
