...
#colmode-group,#namemode-group,#layout-orient-group,.seg-group{display:flex;box-shadow:0 1px 3px rgba(0,0,0,.08);border-radius:6px}
#colmode-group .diag-btn,#namemode-group .diag-btn,#layout-orient-group .diag-btn,.seg-group .diag-btn{box-shadow:none;border-radius:0;font-size:11px;margin-left:-1px}
#colmode-group .diag-btn:first-child,#namemode-group .diag-btn:first-child,#layout-orient-group .diag-btn:first-child,.seg-group .diag-btn:first-child{border-radius:6px 0 0 6px;margin-left:0}
#colmode-group .diag-btn:last-child,#namemode-group .diag-btn:last-child,#layout-orient-group .diag-btn:last-child,.seg-group .diag-btn:last-child{border-radius:0 6px 6px 0}
#layout-orient-group .lo-label{display:flex;align-items:center;padding:0 6px;font-size:10px;font-weight:600;
  color:#64748b;background:#f8fafc;border:1px solid #e2e8f0;border-right:0;border-radius:6px 0 0 6px;height:30px}
body.dark #layout-orient-group .lo-label{color:#94a3b8;background:#1e293b;border-color:#334155}
/* label is the visual first cell; first real button loses the left-radius from the rule above */
#layout-orient-group .lo-btn:first-of-type{border-radius:0;margin-left:0}
#layout-orient-group .lo-btn:last-child{border-radius:0 6px 6px 0}
...
/* ── legend ── */
/* The legend is a passive HUD overlay: its panel floats over the canvas and
   must not swallow clicks/drags meant for the tables underneath it (at
   1600px it spans ~753x527 and covers real nodes). pointer-events:none on
   the panel is what makes that true — #legend-body's own :none below was
   always dead on its own, because a click passing through a child still
   landed on this parent. Everything interactive re-enables :auto
   explicitly: the collapse header here, the note links/entries further
   down. */
...
#legend-notes{margin-top:6px;padding-top:6px;border-top:1px solid #f1f5f9;pointer-events:none}
#legend-notes:empty,#legend-notes[style*="display: none"]{border-top:none;padding-top:0;margin-top:0}
#legend-notes a,#legend-notes .note-entry{pointer-events:auto}
...
/* kept (auto-expand was turned off, but this table was left on screen
   rather than yanked away) — a visibly different, finer dash than 'auto'
   so the two dashed states never look identical side by side */
.er-node.kept .n-hdr{fill:#52606d}
.er-node.kept .n-bg{stroke-dasharray:2 2}
...
/* kept: distinct from both the plain dark header and 'auto' — same finer
   dash pattern carries over from the light-mode rule above */
body.dark .er-node.kept .n-hdr{fill:#3f4b5c}
...
body.dark .grid-note-btn {
  border-color: #475569;
  color: #94a3b8;
}
body.dark .grid-note-btn:hover {
  background: #334155;
  color: #f8fafc;
}
.grid-note-edit-btn {
...
body.dark .grid-inline-editor textarea {
  background: #0f172a;
  color: #f8fafc;
  border-color: #60a5fa;
}
...
body.dark .config-dirty-badge.clean {
  background: #14532d;
  color: #dcfce7;
  border-color: #15803d;
}
...
<div id="topbar">
  ...
  <label id="ae-label" title="ON: automatically shows tables related to every checked root. OFF: stops expanding further — tables it already showed stay on screen (dashed KEPT) until you promote or remove them individually.">
  ...
  <div id="center-pane">
    ...
    <div id="legend">
      ...
      <div id="legend-body">
        ...
        <div class="lhint">Framework association names (has_many etc.) appear in the right pane<br>
          ...
          Esc: exit focus</div>
        <div id="legend-notes" style="display:none"></div>
      </div>
    </div>
    ...
    <div id="diagram-toolbar">
      ...
      <div class="tb-group">
        ...
        <div id="layout-orient-group" title="Layout orientation for the overview (Auto / Vertical / Horizontal). Independent of Direction (Both/Deps/Dependents). Overview Auto-tidy and ↺ only — not Focus mode.">
          <span class="lo-label">Layout</span>
          <button type="button" class="diag-btn lo-btn" data-lo="auto" title="Auto: pick Vertical or Horizontal for the current viewport on overview full layouts (near-ties keep Vertical)">Auto</button>
          <button type="button" class="diag-btn lo-btn active" data-lo="vertical" title="Vertical: top/bottom depth rows (compatibility default)">↕ Vertical</button>
          <button type="button" class="diag-btn lo-btn" data-lo="horizontal" title="Horizontal: hub-centered left/right branches (overview full layouts only)">↔ Horizontal</button>
        </div>
      ...
      <div class="modal-ctrls">
        ...
        <div class="modal-search-wrapper">
          <select id="grid-search-scope" title="Filter target scope" style="border:1px solid #cbd5e1;border-radius:4px 0 0 4px;font-size:12px;padding:4px 6px;background:#f8fafc;color:#0f172a;border-right:none">
            ...
            <option value="table" selected>Table Name</option>
        ...
        <button class="diag-btn" id="btn-grid-newtab" title="Open standalone read-only Data Dictionary view in a new browser tab">Open read-only view ↗</button>
  ...
  <div class="modal-dialog" style="max-width:480px;height:auto">
    ...
    <div class="modal-bdy" style="padding:16px;display:flex;flex-direction:column;gap:12px">
      ...
      <div style="display:flex;flex-direction:column;gap:8px;margin-top:8px">
        ...
        <button class="diag-btn" id="btn-unsaved-discard" style="background:#fee2e2;color:#991b1b;border:1px solid #fca5a5;padding:8px 16px">
  ...
  <div class="modal-dialog" style="max-width:500px;height:auto">
    ...
    <div class="modal-bdy" style="padding:16px;display:flex;flex-direction:column;gap:12px">
      <div style="display:flex;flex-direction:column;gap:4px">
        <label style="font-size:12px;font-weight:600">Table Name (Physical)</label>
        <input type="text" id="prop-table-name" placeholder="e.g. proposed_orders" style="padding:6px;border:1px solid #cbd5e1;border-radius:4px" />
      </div>
      <div style="display:flex;flex-direction:column;gap:4px">
        <label style="font-size:12px;font-weight:600">Logical Name (Optional)</label>
        <input type="text" id="prop-table-logical" placeholder="e.g. 注文提案テーブル" style="padding:6px;border:1px solid #cbd5e1;border-radius:4px" />
      </div>
      <div style="display:flex;flex-direction:column;gap:4px">
      ...
      <div style="display:flex;flex-direction:column;gap:4px">
        ...
        <input type="text" id="prop-table-group" placeholder="e.g. Core Domain" style="padding:6px;border:1px solid #cbd5e1;border-radius:4px" />
      </div>
      <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:8px">
        <button class="diag-btn" id="btn-prop-table-cancel">Cancel</button>
        <button class="diag-btn" id="btn-prop-table-save" style="background:#2563eb;color:#fff;border:none;padding:6px 16px;font-weight:600">Add Table</button>
      </div>
    </div>
  </div>
</div>
...
<div id="proposed-column-modal" class="modal-backdrop hidden">
  <div class="modal-dialog" style="max-width:500px;height:auto">
    ...
    <div class="modal-bdy" style="padding:16px;display:flex;flex-direction:column;gap:12px">
      <div style="display:flex;flex-direction:column;gap:4px">
        <label style="font-size:12px;font-weight:600">Target Table</label>
        <select id="prop-col-table-sel" style="padding:6px;border:1px solid #cbd5e1;border-radius:4px"></select>
      </div>
      <div style="display:flex;flex-direction:column;gap:4px">
        <label style="font-size:12px;font-weight:600">Column Name</label>
        <input type="text" id="prop-col-name" placeholder="e.g. status_code" style="padding:6px;border:1px solid #cbd5e1;border-radius:4px" />
      </div>
      <div style="display:flex;flex-direction:column;gap:4px">
        <label style="font-size:12px;font-weight:600">Logical Name (Optional)</label>
        <input type="text" id="prop-col-logical" placeholder="e.g. ステータスコード" style="padding:6px;border:1px solid #cbd5e1;border-radius:4px" />
      </div>
      <div style="display:flex;gap:12px">
        <div style="flex:1;display:flex;flex-direction:column;gap:4px">
          <label style="font-size:12px;font-weight:600">Data Type</label>
          <input type="text" id="prop-col-type" placeholder="e.g. varchar(255)" style="padding:6px;border:1px solid #cbd5e1;border-radius:4px" value="varchar(255)" />
        </div>
        <div style="flex:1;display:flex;flex-direction:column;gap:4px">
          <label style="font-size:12px;font-weight:600">Nullable</label>
          <select id="prop-col-nullable" style="padding:6px;border:1px solid #cbd5e1;border-radius:4px">
      ...
      <div style="display:flex;flex-direction:column;gap:4px">
        <label style="font-size:12px;font-weight:600">Comment (Optional)</label>
        <input type="text" id="prop-col-comment" placeholder="e.g. ToBe proposed status" style="padding:6px;border:1px solid #cbd5e1;border-radius:4px" />
      </div>
      <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:8px">
        <button class="diag-btn" id="btn-prop-col-cancel">Cancel</button>
        <button class="diag-btn" id="btn-prop-col-save" style="background:#2563eb;color:#fff;border:none;padding:6px 16px;font-weight:600">Add Column</button>
      </div>
    </div>
  </div>
</div>
...
<div id="note-edit-modal" class="modal-backdrop hidden">
  <div class="modal-dialog" style="max-width:560px;height:auto;max-height:85vh">
    ...
    <div class="modal-bdy" style="padding:16px;display:flex;flex-direction:column;gap:12px">
      ...
      <div style="display:flex;flex-direction:column;gap:4px" id="note-target-source-group">
        <label style="font-size:12px;font-weight:600">Source Table</label>
        <input type="text" id="note-edit-source-table" style="padding:6px;border:1px solid #cbd5e1;border-radius:4px" placeholder="e.g. posts" />
      </div>
      <div style="display:flex;flex-direction:column;gap:4px" id="note-target-rel-table-group">
        <label style="font-size:12px;font-weight:600">Target Table</label>
        <input type="text" id="note-edit-target-table" style="padding:6px;border:1px solid #cbd5e1;border-radius:4px" placeholder="e.g. users" />
      </div>
      <div style="display:flex;flex-direction:column;gap:4px" id="note-target-fk-group">
        <label style="font-size:12px;font-weight:600">Foreign Key Column (Optional)</label>
        <input type="text" id="note-edit-foreign-key" style="padding:6px;border:1px solid #cbd5e1;border-radius:4px" placeholder="e.g. user_id" />
      </div>
      <div style="display:flex;flex-direction:column;gap:4px">
...
// tables that were shown only by auto-expand at the moment it was turned
// OFF — kept visible so switching Auto-expand off reads as "stop expanding
// further" rather than "throw away what was already shown". Distinct from
// excludedTables (still unchecked) and noAutoExpandRoot (only meaningful
// while auto-expand is actually running its BFS): a retained table is
// neither an explicit selection nor a fresh BFS root. Cleared whenever
// auto-expand is turned back ON (getDisplayTables() ignores it while ON —
// its own BFS recomputes from scratch), or when the table is promoted,
// removed, or the overview is reset via All/None/a saved view.
let retainedExpandedTables = new Set();
let autoLayout     = false; // re-layout automatically whenever the display set changes
// Overview layout orientation. Independent of expandDir (relation traversal
// Direction: Both/Deps/Dependents). Missing/invalid values fall back to
// 'vertical' for old localStorage, named views, and share links.
//
// Placement engines: Vertical, Horizontal, and Auto (viewport-scored) are
// implemented for overview full layouts. Focus always uses Vertical.
let layoutOrientation = 'vertical'; // 'auto' | 'vertical' | 'horizontal'
const LAYOUT_ORIENTATION_IMPLEMENTED = { auto: true, vertical: true, horizontal: true };
// Which placement engine gridLayout last *chose* when it committed a snap
// (the Layout policy may still be 'auto'). Not a live description of the
// current coordinates: focus exit restores basePos, Undo restores a prior
// position snapshot, and incremental Auto-tidy-off placement never runs
// gridLayout — so after those paths lastLayoutEngine can disagree with
// the geometry on screen. Diagnostics and e2e only; not persisted.
let lastLayoutEngine = 'vertical'; // 'vertical' | 'horizontal'
...
// ── LocalStorage ──────────────────────────────────────────────────────────
// Valid layout orientations; anything else (including null/undefined) → vertical.
function normalizeLayoutOrientation(v){
  return (v==='auto'||v==='horizontal'||v==='vertical') ? v : 'vertical';
}
function saveState() {
  setLS(LS('excl'), JSON.stringify([...excludedTables]));
  setLS(LS('hid'),  JSON.stringify([...hiddenTables]));
  setLS(LS('ret'),  JSON.stringify([...retainedExpandedTables]));
...
function loadState() {
  ...
  // old saves predate this field entirely, and ret:null / ret:{} / a
  // corrupted value / an unknown table name must all fall back to an empty
  // set without ever aborting the rest of loadState()
  try {
    const raw = JSON.parse(localStorage.getItem(LS('ret')) || '[]');
    retainedExpandedTables = new Set(Array.isArray(raw) ? raw.filter(t => DATA.tables[t]) : []);
  } catch { retainedExpandedTables = new Set(); }
  ...
  // absent / null / unknown → vertical (compatibility default for pre-Layout saves)
  layoutOrientation = normalizeLayoutOrientation(localStorage.getItem(LS('lo')));
  ...
  // retainedExpandedTables only has meaning while auto-expand is off — a
  // save made (or hand-edited) with ae:true carrying a stale/foreign ret
  // must not resurrect it once auto-expand's own BFS takes over again
  if (autoExpand) retainedExpandedTables.clear();
  // a table that's explicitly checked wins over a contradictory "also
  // retained" entry — retained never overrides an explicit selection
  for (const t of retainedExpandedTables) if (!excludedTables.has(t)) retainedExpandedTables.delete(t);
}
...
// With auto-expand off, tables retained from the last time it WAS on stay
// visible too (see retainedExpandedTables above) — turning the toggle off
// stops further expansion, it does not retract what's already shown.
...
function getDisplayTables() {
  if (!focusedTable) {
    if (!autoExpand) {
      return allTables().filter(t => !hiddenTables.has(t)
        && (!excludedTables.has(t) || retainedExpandedTables.has(t)));
    }
...
// Pure classifier (no side effects) for how `name` is currently shown in the
// non-focused Overview — shared by drawNode() and renderTableList() so the
// canvas and the list never disagree about root/checked/auto/retained/
// excluded/banned state. `displaySet` is getDisplayTables() computed ONCE
// by the caller (a Set) — this must never call getDisplayTables() itself,
// or a full-list render becomes O(n²).
function overviewDisplayKind(name, displaySet) {
  if (hiddenTables.has(name)) return 'banned';
  if (!displaySet.has(name)) return 'excluded';
  if (!excludedTables.has(name)) {
    return (autoExpand && !noAutoExpandRoot.has(name)) ? 'root' : 'checked';
  }
  return retainedExpandedTables.has(name) ? 'retained' : 'auto';
}
...
// Least-travel push out of an axis-aligned obstacle box. Vertical layouts
// keep the historical down/right/left set (rows grow top→bottom). Horizontal
// layouts also allow up so side-by-side branches are not systematically shoved
// downward. Tie-break order is fixed for determinism: down > right > left > up.
function leastTravelClearancePos(p, s, box, allowUp){
  const x0=p.x-s.w/2, y0=p.y-s.h/2, x1=p.x+s.w/2, y1=p.y+s.h/2;
  const pad=20;
  const cands=[
    {ord:0, cost:box.y1-y0, pos:{x:p.x, y:box.y1+pad+s.h/2}},       // down
    {ord:1, cost:box.x1-x0, pos:{x:box.x1+pad+s.w/2, y:p.y}},       // right
    {ord:2, cost:x1-box.x0, pos:{x:box.x0-pad-s.w/2, y:p.y}},       // left
  ];
  if(allowUp) cands.push({ord:3, cost:y1-box.y0, pos:{x:p.x, y:box.y0-pad-s.h/2}});
  cands.sort((a,b)=> (a.cost-b.cost) || (a.ord-b.ord));
  return cands[0].pos;
}
// Set true for the duration of a Horizontal runPass (including Auto's
// Horizontal candidate) so group/residual clearance can push up without
// permanently coupling to the user's Layout policy.
let _runPassIsHorizontal=false;
function layoutAllowsUpClearance(){
  // focus always uses the Vertical engine; overview Horizontal (or Auto's
  // Horizontal evaluation pass) may push up as least-travel
  return _runPassIsHorizontal && !focusedTable;
}

function resolveResidualOverlaps(tables){
  const memberSet=new Set(GROUPS.flatMap(g=>g.tables));
  const allowUp=layoutAllowsUpClearance();
  for(let pass=0; pass<3; pass++){
    ...
    for(let i=0;i<tables.length;i++){
      ...
      for(let j=0;j<tables.length;j++){
        ...
        nodePos[t]=leastTravelClearancePos(p, s, {x0:ox0,y0:oy0,x1:ox1,y1:oy1}, allowUp);
        moved=true;
...
// Auto orientation tolerances: if fitScale and edgeLen differ by at most
// these *relative* amounts, treat candidates as effectively tied and prefer
// Vertical (anti-jitter / compatibility default). Documented constants —
// not knobs. Absolute floors avoid division noise on tiny values.
const AUTO_FIT_SCALE_REL=0.08;
const AUTO_EDGE_LEN_REL=0.05;
function layoutCandidatesEffectivelyTied(a, b){
  if(!!a.overlap!==!!b.overlap) return false;
  const maxFit=Math.max(a.fitScale, b.fitScale, 1e-6);
  if(Math.abs(a.fitScale-b.fitScale)/maxFit>AUTO_FIT_SCALE_REL) return false;
  const maxE=Math.max(a.edgeLen, b.edgeLen, 1e-6);
  if(Math.abs(a.edgeLen-b.edgeLen)/maxE>AUTO_EDGE_LEN_REL) return false;
  return true;
}

// rank layout candidates {fitScale, overlap, edgeLen[, orientation]}:
// no-overlap always beats any-overlap, then larger fitScale, then shorter
// edgeLen, then declaration order. If EVERY candidate still overlaps, fall
// back to the first candidate (safe Vertical-first when Auto builds the
// list as V then H). When orientations are mixed (Auto), a Horizontal win
// that is only within tolerance of a clean Vertical candidate yields to
// Vertical instead.
function pickBestLayoutCandidate(results){
  ...
  if(best.orientation==='horizontal'){
    // prefer the best clean Vertical that is effectively tied with best
    for(const r of ranked){
      if(r.overlap || r.orientation!=='vertical') continue;
      if(layoutCandidatesEffectivelyTied(best, r)) return r;
    }
  }
  return best;
}
...
function gridLayout(tables, preferredHub, adaptive) {
  ...
  // Overview Horizontal uses a real left/right branch engine (not an X/Y
  // swap). Focus always keeps the Vertical hub-spoke path via preferredHub.
  // Auto evaluates a bounded set of Vertical + Horizontal finals and picks
  // deterministically (Vertical on near-ties) — only when gridLayout already
  // runs a full overview layout (↺, Layout change, Auto-tidy full pack).
  const forceVertical = !!preferredHub; // focus mode
  const orientMode = forceVertical ? 'vertical' : layoutOrientation; // auto|vertical|horizontal
  ...
  // Horizontal overview placement: hub near the component center, depth-1
  // branches partitioned left/right by subtree load, deeper tables progress
  // outward on X within their branch root's side. Real node widths/heights
  // stay on their native axes (never an X/Y swap of the Vertical engine).
  //
  // Branch ownership: BFS tree from the hub; each depth-2+ table is owned by
  // the unique depth-1 ancestor on that tree. When BFS discovers a node from
  // several same-depth neighbors, the first parent in alphabetical frontier
  // expansion order wins (neighbors of each frontier node are sorted).
  // Cross-branch and cycle edges never reassign ownership.
  //
  // Side partition: largest load first → lighter side. On equal load, prefer
  // dependency branches left / dependents right; neutral or still tied →
  // left. Equal-load same-role fans therefore alternate ~3/3 after the first
  // role-biased pick (subsequent assignments follow the lighter side).
  function layoutComponentHorizontal(comp){
    const hub=(preferredHub && comp.includes(preferredHub)) ? preferredHub
      : comp.slice().sort((a,b)=>(adj.get(b).size-adj.get(a).size)||a.localeCompare(b))[0];
    const depMap=new Map([[hub,0]]);
    ...
    while(frontier.length){
      const next=[];
      frontier.slice().sort().forEach(name=>{
        [...adj.get(name)].sort().forEach(n=>{
          if(depMap.has(n)) return;
          depMap.set(n, depMap.get(name)+1);
          treeParent.set(n, name);
          next.push(n);
        });
      });
      frontier=next;
    ...
    function branchOwner(t){
      if(t===hub) return null;
      let cur=t;
      while(treeParent.has(cur) && (depMap.get(cur)||0)>1) cur=treeParent.get(cur);
      return cur;
    }

    const d1=[...depMap.entries()].filter(([,d])=>d===1).map(([t])=>t).sort();
    const branchMembers=new Map(d1.map(r=>[r,[r]]));
    depMap.forEach((d,t)=>{
      if(d<=1) return;
      const own=branchOwner(t);
      if(own && branchMembers.has(own)) branchMembers.get(own).push(t);
    });

    function branchLoad(root){
      let load=0;
      (branchMembers.get(root)||[root]).forEach(t=>{
        const s=nodeSize[t]||calcSize(t);
        const d=depMap.get(t)||1;
        // area + perimeter + depth — whole subtree, not just the depth-1 node
        load += s.w*s.h + (s.w+s.h)*20 + d*80 + 500;
      });
      return load;
    }

    // hub depends on root → root is a dependency (prefer left);
    // root depends on hub → root is a dependent (prefer right)
    function branchRole(root){
      let preferLeft=false, preferRight=false;
      (DATA.tables[hub]?.associations||[]).forEach(a=>{
        if(a.polymorphic||a.target!==root) return;
        if(a.type==='belongs_to') preferLeft=true;
        if(a.type==='has_many'||a.type==='has_one') preferRight=true;
      });
      (DATA.tables[root]?.associations||[]).forEach(a=>{
        if(a.polymorphic||a.target!==hub) return;
        if(a.type==='belongs_to') preferRight=true;
        if(a.type==='has_many'||a.type==='has_one') preferLeft=true;
      });
      if(preferLeft && !preferRight) return 'dep';
      if(preferRight && !preferLeft) return 'dependent';
      return 'neutral';
    }
    ...
    d1.slice().sort((a,b)=>{
      const la=branchLoad(a), lb=branchLoad(b);
      return (lb-la) || a.localeCompare(b);
    }).forEach(root=>{
      const L=branchLoad(root);
      const role=branchRole(root);
      if(leftLoad<rightLoad){ left.push(root); leftLoad+=L; }
      else if(rightLoad<leftLoad){ right.push(root); rightLoad+=L; }
      else if(role==='dependent'){ right.push(root); rightLoad+=L; }
      else { left.push(root); leftLoad+=L; } // dep, neutral, or still tied → left
    });

    const hubS=nodeSize[hub]||calcSize(hub);
    nodePos[hub]={x:0, y:0};

    function placeSide(roots, sign){
      if(!roots.length) return;
      const ordered=roots.slice().sort((a,b)=>a.localeCompare(b));
      const measured=ordered.map(root=>{
        const members=branchMembers.get(root)||[root];
        const byD={};
        members.forEach(t=>{ const d=depMap.get(t); (byD[d]=byD[d]||[]).push(t); });
        const depths=Object.keys(byD).map(Number).sort((a,b)=>a-b);
        const colMeta=depths.map(d=>{
          const nodes=byD[d].slice().sort();
          const colH=nodes.reduce((s,t)=>s+(nodeSize[t]||calcSize(t)).h+gapY,0)-gapY;
          const colW=Math.max(...nodes.map(t=>(nodeSize[t]||calcSize(t)).w));
          return {d, nodes, colH, colW};
        });
        const branchH=Math.max(...colMeta.map(c=>c.colH), 0);
        return {root, colMeta, branchH};
      });
      const totalH=measured.reduce((s,m)=>s+m.branchH,0)+gapY*Math.max(0,measured.length-1);
      let yCursor=-totalH/2;
      measured.forEach(m=>{
        const blockMid=yCursor+m.branchH/2;
        let xEdge=sign*(hubS.w/2+gapX);
        m.colMeta.forEach(col=>{
          const cx=sign>0 ? xEdge+col.colW/2 : xEdge-col.colW/2;
          let y=blockMid-col.colH/2;
          col.nodes.forEach(t=>{
            const s=nodeSize[t]||calcSize(t);
            nodePos[t]={x:cx, y:y+s.h/2};
            y+=s.h+gapY;
          });
          xEdge+=sign*(col.colW+gapX);
        });
        yCursor+=m.branchH+gapY;
      });
    }
  ...
  // Lay the whole diagram out once under a given row-width policy + engine
  // and score the result — mutates the live nodePos/nodeSize as it goes
  // (layoutComponent always writes fresh positions for every table in
  // `tables`, never reads stale ones, so passes don't contaminate each
  // other) but returns a snapshot rather than leaving its result committed;
  // only the winning candidate's snapshot is written back by the caller.
  // `horizontal=true` selects layoutComponentHorizontal; false = Vertical.
  function runPass(policy, horizontal){
    widthPolicy=policy;
    _runPassIsHorizontal=!!horizontal;
    // shelf-pack component boxes (largest first) toward the viewport shape
    const boxes=comps.map(c=> horizontal ? layoutComponentHorizontal(c) : layoutComponent(c));
    ...
    _runPassIsHorizontal=false;
    return {
      snap, fitScale, overlap, edgeLen,
      orientation: horizontal ? 'horizontal' : 'vertical',
    };
  }

  // Candidate set (bounded, never open-ended):
  //   Vertical explicit / focus: adaptive row-width policies when requested
  //   Horizontal explicit: one Horizontal pass
  //   Auto: Vertical policies first, then one Horizontal — Vertical-first
  //         order also makes the all-overlap fallback safe
  let candidates;
  try{
    if(orientMode==='horizontal'){
      candidates=[runPass(1, true)];
    } else if(orientMode==='auto'){
      const vPolicies=adaptive ? [1, 1.6, 'max'] : [1];
      candidates=vPolicies.map(p=>runPass(p, false));
      candidates.push(runPass(1, true));
    } else {
      const vPolicies=adaptive ? [1, 1.6, 'max'] : [1];
      candidates=vPolicies.map(p=>runPass(p, false));
    }
  ...
  lastLayoutEngine=best.orientation||'vertical';
...
// Tables actually rendered by the last renderDiagram() call (kept current
// by renderDiagram() itself, so it stays right regardless of which caller
// triggered that render — see refreshView() below). Deliberately NOT
// derived from nodePos's keys: nodePos is a coordinate cache that outlives
// a table leaving the display set (so a manually-dragged table reappears
// where it was left, with Auto-tidy off) — using its keys as "was this
// already on screen" conflated "has a remembered position" with "was
// actually visible a moment ago", so a table that had been unchecked and
// was now being re-checked could be wrongly treated as not-newly-visible
// (it still had a stale leftover entry in nodePos from before), silently
// skipping the fit and leaving it wherever gridLayout happened to drop it
// — potentially off-screen, needing a manual scroll/pan to find.
...
function refreshView(forceFit, forceRelayout){
  ...
  //
  // Scoped to newly-visible tables only (not the full display set): once a
  ...
  // a table, which brings nothing new into view) silently zoomed back out
  // to fit-all on the very next interaction. A removal-only change must
  // never force a fit; Auto-tidy re-packing the *entire* display set always
  // does, since the new layout can land anywhere.
  //
  // Going from 0 displayed tables to 1+ always fits too, even though that
  // lone table is technically "newly visible" either way — called out
  // separately because it's the common "None, then check one table" path,
  // and it must never depend on isDisplayInView() guessing right about an
  // empty-viewport starting point.
  const currentDisplayed = getDisplayTables();
  const newlyVisible = currentDisplayed.filter(t => !previouslyDisplayed.has(t));
  const wasEmptyBefore = previouslyDisplayed.size===0 && currentDisplayed.length>0;
  if(forceFit || relayoutedAll || wasEmptyBefore || !isDisplayInView(newlyVisible))
...
function renderDiagram() {
  erMain.innerHTML='';
  const tables = getDisplayTables();
  const displaySet = new Set(tables); // passed into drawNode() so
    // overviewDisplayKind() doesn't recompute getDisplayTables() per node
  ...
  tables.forEach(n => drawNode(nodeG, n, displaySet));
  ...
  // keep this in sync regardless of which caller invoked renderDiagram()
  // directly (not every caller goes through refreshView()) — see its
  // declaration for why this must not be nodePos's keys instead.
  lastDisplayedTables = displaySet;
}
...
// frame with least travel (see leastTravelClearancePos). Vertical keeps
// down/right/left; overview Horizontal also allows up. A pushed table can
// in principle end up overlapping some other unrelated node; that residual
// risk is accepted the same way gridLayout's own placement heuristics already
// accept it elsewhere — perfect collision-free packing is not this pass's
// job, only keeping tables out of OTHER tables' group frames.
  ...
  for(let pass=0; pass<3; pass++){
    ...
    GROUPS.forEach(g=>{
      ...
      placedTables.forEach(t=>{
        ...
        nodePos[t]=leastTravelClearancePos(p, s, bbox, allowUp);
        moved=true;
      });
    });
    if(!moved) break;
...
function drawNode(parent, name, displaySet) {
  ...
  // overview only (not focused): roots (checked BFS roots while auto-expand
  // is on) get a ✓ badge; tables shown only because expansion pulled them in
  // get the dashed 'auto' style; tables kept visible after auto-expand was
  // turned OFF get the dashed 'kept' style — see overviewDisplayKind().
  const kind = focusedTable ? null : overviewDisplayKind(name, displaySet || new Set([name]));
  const isRoot = kind === 'root';
  const isAuto = kind === 'auto';
  const isKept = kind === 'retained';
  const isWordHit = wordHit(name);
  const hasLogical = !!logicalName(name);
  const g = svgEl('g', {
    class: 'er-node' + (selectedTables.has(name)?' sel':'') + (name===focusedTable?' center':'')
      + (isAuto?' auto':'') + (isKept?' kept':'') + ringCls
  ...
  if(isRoot){
    ...
    g.appendChild(rb);
  } else if(isAuto || isKept){
    // dashed table: shown but not itself checked — either 'auto' (some other
    // root is currently pulling it in live; its list checkbox is disabled,
    // see renderTableList) or 'kept' (auto-expand pulled it in before being
    // turned OFF, and it was left on screen rather than yanked away; its
    // list checkbox is enabled, unchecked). Either way this ＋ is the one
    // direct way to promote it to an explicit root, so it keeps its place
    // even if auto-expand's roots/depth/direction/on-off state later change
    // and would otherwise have dropped it.
...
// ── Straight-first orthogonal edge routing ─────────────────────────────────
// Preferred order for non-self edges:
//   1. direct straight segment (even when multi-assoc or diagonal)
//   2. one-bend orthogonal L
//   3. two-bend orthogonal dogleg (H-first and V-first, small lane set)
//   4. bounded Bézier fallback only when nothing orthogonal clears
// Association count never forces a bend. Group frames are not obstacles.
const EDGE_CLEAR=8;     // expanded-table clearance margin (px)
const EDGE_MIN_LEG=10;  // minimum first/last orth leg to avoid marker zigzags
const EDGE_LANE=20;     // dogleg lane offset past expanded bounds
const CURVE_T=0.2;      // Bézier control fraction (fallback only)

function edgeExpandedBox(name){
  const p=nodePos[name], s=nodeSize[name];
  if(!p||!s) return null;
  return {x0:p.x-s.w/2-EDGE_CLEAR, x1:p.x+s.w/2+EDGE_CLEAR,
          y0:p.y-s.h/2-EDGE_CLEAR, y1:p.y+s.h/2+EDGE_CLEAR};
}
function sidePt(cx,cy,w,h,side){
  const hw=w/2+2, hh=h/2+2;
  if(side==='L') return {x:cx-hw, y:cy};
  if(side==='R') return {x:cx+hw, y:cy};
  if(side==='T') return {x:cx, y:cy-hh};
  return {x:cx, y:cy+hh}; // B
}
...
// Closed segment–segment intersection (including touching endpoints).
function segSeg(ax,ay,bx,by, cx,cy,dx,dy){
  const den=(bx-ax)*(dy-cy)-(by-ay)*(dx-cx);
  if(Math.abs(den)<1e-9){
    // collinear/parallel: treat as non-crossing for routing (boxes still catch)
    return false;
  }
...
// True if the open-interior of the expanded box is crossed by the segment,
// or either endpoint lies inside the closed expanded box.
function segmentHitsBox(ax,ay,bx,by, b){
  if(Math.max(ax,bx)<b.x0-1e-6||Math.min(ax,bx)>b.x1+1e-6||
     Math.max(ay,by)<b.y0-1e-6||Math.min(ay,by)>b.y1+1e-6) return false;
  if(pointInBox(ax,ay,b)||pointInBox(bx,by,b)) return true;
  return segSeg(ax,ay,bx,by, b.x0,b.y0,b.x1,b.y0)
      || segSeg(ax,ay,bx,by, b.x1,b.y0,b.x1,b.y1)
      || segSeg(ax,ay,bx,by, b.x1,b.y1,b.x0,b.y1)
      || segSeg(ax,ay,bx,by, b.x0,b.y1,b.x0,b.y0);
}
function polylineHitsObstacles(pts, edge){
  for(let i=0;i<pts.length-1;i++){
    const a=pts[i], b=pts[i+1];
    for(const t of edgeObstacles){
      if(t===edge.source||t===edge.target) continue;
      const box=edgeExpandedBox(t);
      if(!box) continue;
...
// Endpoint boxes are excluded from ordinary obstacle checks, but an
// orthogonal elbow must not enter either endpoint through one side and leave
// through another.  All anchors sit 2px outside the real node box, so any
// intersection with the unexpanded endpoint interior is invalid.
function polylineHitsEndpointInteriors(pts, edge){
  for(const t of [edge.source, edge.target]){
    const p=nodePos[t], s=nodeSize[t];
    if(!p||!s) continue;
    const box={x0:p.x-s.w/2, x1:p.x+s.w/2,
               y0:p.y-s.h/2, y1:p.y+s.h/2};
    for(let i=0;i<pts.length-1;i++){
      const a=pts[i], b=pts[i+1];
      if(segmentHitsBox(a.x,a.y,b.x,b.y,box)) return true;
    }
...
// Min distance from polyline AABB-per-segment to any non-endpoint expanded
// box (0 if hits). Cheap and deterministic — only used for tie-breaking.
function polylineMinClearance(pts, edge){
  let min=Infinity;
  for(let i=0;i<pts.length-1;i++){
    const a=pts[i], b=pts[i+1];
    for(const t of edgeObstacles){
      ...
      const box=edgeExpandedBox(t);
      if(!box) continue;
      if(segmentHitsBox(a.x,a.y,b.x,b.y, box)) return 0;
      const sx0=Math.min(a.x,b.x), sx1=Math.max(a.x,b.x);
      const sy0=Math.min(a.y,b.y), sy1=Math.max(a.y,b.y);
      const dx=Math.max(0, Math.max(box.x0-sx1, sx0-box.x1));
      const dy=Math.max(0, Math.max(box.y0-sy1, sy0-box.y1));
...
function cleanWaypoints(pts){
  ...
  // drop collinear intermediate bends
  if(out.length>=3){
    const slim=[out[0]];
    for(let i=1;i<out.length-1;i++){
      const a=slim[slim.length-1], b=out[i], c=out[i+1];
      const cross=(b.x-a.x)*(c.y-a.y)-(b.y-a.y)*(c.x-a.x);
      if(Math.abs(cross)>1e-3) slim.push(b);
    }
...
function routeBends(pts){ return Math.max(0, pts.length-2); }
function firstLastLegsOk(pts){
  ...
  // straight single segment: no "leg" minimum beyond length > 0
  if(pts.length===2) return a>0.5;
  return a>=EDGE_MIN_LEG-0.5 && b>=EDGE_MIN_LEG-0.5;
}
function pathDFromPts(pts){
  let d=`M ${pts[0].x} ${pts[0].y}`;
  for(let i=1;i<pts.length;i++) d+=` L ${pts[i].x} ${pts[i].y}`;
  return d;
}
function longestSegmentMid(pts){
  let bestI=0, bestL=-1;
  for(let i=0;i<pts.length-1;i++){
...
// Axis-aligned rectangle overlap (closed).
function rectsOverlap(ax0,ay0,ax1,ay1, bx0,by0,bx1,by1){
  return ax0<bx1 && ax1>bx0 && ay0<by1 && ay1>by0;
}
// Place a through-label of size (lw × lh) whose *center* starts at `seed`.
// Nudges the full label rectangle off every displayed table box (including
// edge endpoints) — not just the center point — so the background pill
// cannot overlap a node. Used for both orthogonal routes and the Bézier
// fallback (drawEdge always calls this with the real thr string width).
function throughLabelPos(seed, lw, lh){
  const halfW=Math.max(1, lw/2), halfH=Math.max(1, (lh||14)/2);
  const pad=4;
  let cx=seed.x, cy=seed.y;
  function labelHitsAny(x,y){
    const lx0=x-halfW, lx1=x+halfW, ly0=y-halfH, ly1=y+halfH;
    for(const t of edgeObstacles){
      const p=nodePos[t], s=nodeSize[t];
      if(!p||!s) continue;
      const x0=p.x-s.w/2, x1=p.x+s.w/2, y0=p.y-s.h/2, y1=p.y+s.h/2;
      if(rectsOverlap(lx0,ly0,lx1,ly1, x0,y0,x1,y1)) return {x0,y0,x1,y1};
    }
    return null;
  }
  // Bounded least-travel pushes against each overlapping table. Prefer a
  // candidate that clears *all* tables; fall back to clearing just the hit.
  for(let iter=0; iter<24; iter++){
    const hit=labelHitsAny(cx,cy);
    if(!hit) break;
    const opts=[
      {x:hit.x0-halfW-pad, y:cy},
      {x:hit.x1+halfW+pad, y:cy},
      {x:cx, y:hit.y0-halfH-pad},
      {x:cx, y:hit.y1+halfH+pad},
      {x:hit.x0-halfW-pad, y:hit.y0-halfH-pad},
      {x:hit.x1+halfW+pad, y:hit.y0-halfH-pad},
      {x:hit.x0-halfW-pad, y:hit.y1+halfH+pad},
      {x:hit.x1+halfW+pad, y:hit.y1+halfH+pad},
    ];
    let bestAll=null, bestAllD=Infinity, bestOne=null, bestOneD=Infinity;
    for(const o of opts){
      const d=Math.hypot(o.x-cx, o.y-cy);
      const hitsHit=rectsOverlap(o.x-halfW,o.y-halfH,o.x+halfW,o.y+halfH,
                                  hit.x0,hit.y0,hit.x1,hit.y1);
      if(hitsHit) continue;
      if(d<bestOneD){ bestOneD=d; bestOne=o; }
      if(!labelHitsAny(o.x,o.y) && d<bestAllD){ bestAllD=d; bestAll=o; }
    }
    const pick=bestAll||bestOne;
    if(pick){ cx=pick.x; cy=pick.y; }
    else { cx=hit.x0-halfW-pad; cy=hit.y0-halfH-pad; } // corner last resort
  }
  // Dense/nested layouts can consume the bounded local-search budget.  Never
  // return an overlapping pill: deterministically park it beyond the global
  // right edge while retaining the seed Y.  This is intentionally a visual
  // last resort; the common path remains close to the routed edge.
  if(labelHitsAny(cx,cy)){
    let right=-Infinity;
    for(const t of edgeObstacles){
      const p=nodePos[t], s=nodeSize[t];
      if(p&&s) right=Math.max(right, p.x+s.w/2);
    }
...
// Build a fixed small candidate set for a non-self edge, then pick the best.
function routeEdge(edge){
  const sp=nodePos[edge.source], tp=nodePos[edge.target];
  const ss=nodeSize[edge.source]||calcSize(edge.source);
  const ts=nodeSize[edge.target]||calcSize(edge.target);
  const cands=[];
  function add(pts, kind, ord){
    const cleaned=cleanWaypoints(pts);
    if(cleaned.length<2||!firstLastLegsOk(cleaned)) return;
    const blocked=polylineHitsObstacles(cleaned, edge)
      || polylineHitsEndpointInteriors(cleaned, edge);
    cands.push({
      pts:cleaned, kind, ord, blocked,
      bends:routeBends(cleaned),
      len:routeLength(cleaned),
      clear: blocked ? 0 : polylineMinClearance(cleaned, edge),
    });
  }

  // 1. direct straight between center-ray border anchors
  {
    const a=borderPt(sp.x,sp.y,ss.w,ss.h,tp.x,tp.y);
    const b=borderPt(tp.x,tp.y,ts.w,ts.h,sp.x,sp.y);
    add([a,b], 'straight', 0);
  }

  // 2. one-bend L: horizontal-first and vertical-first
  {
    const outH=tp.x>=sp.x?'R':'L';
    const inV=tp.y>=sp.y?'T':'B';
    const a0=sidePt(sp.x,sp.y,ss.w,ss.h,outH);
    const a2=sidePt(tp.x,tp.y,ts.w,ts.h,inV);
    add([a0,{x:a2.x,y:a0.y},a2], 'ortho1-hv', 1);
  }
  {
    const outV=tp.y>=sp.y?'B':'T';
    const inH=tp.x>=sp.x?'L':'R';
    const a0=sidePt(sp.x,sp.y,ss.w,ss.h,outV);
    const a2=sidePt(tp.x,tp.y,ts.w,ts.h,inH);
    add([a0,{x:a0.x,y:a2.y},a2], 'ortho1-vh', 2);
  }

  // 3. two-bend doglegs: H-first / V-first with a small fixed set of lanes.
  // Exit/enter sides face the *lane*. Lanes must sit outside both endpoint
  // boxes (not merely outside mid-obstacles), otherwise the elbow lands
  // inside a source/target and the first leg tunnels through it.
  {
    const epL=Math.min(sp.x-ss.w/2, tp.x-ts.w/2)-EDGE_LANE;
    const epR=Math.max(sp.x+ss.w/2, tp.x+ts.w/2)+EDGE_LANE;
    const midXs=[epL, epR, (sp.x+tp.x)/2];
    for(const t of edgeObstacles){
      if(t===edge.source||t===edge.target) continue;
      const box=edgeExpandedBox(t);
      if(!box) continue;
      const lo=Math.min(sp.x,tp.x), hi=Math.max(sp.x,tp.x);
      if(box.x1<lo-EDGE_LANE||box.x0>hi+EDGE_LANE) continue;
      midXs.push(box.x0-EDGE_LANE, box.x1+EDGE_LANE);
    }
    const seen=new Set();
    midXs.forEach((mx,i)=>{
      // reject lanes that sit inside either endpoint's horizontal span
      if(mx>sp.x-ss.w/2-EDGE_MIN_LEG && mx<sp.x+ss.w/2+EDGE_MIN_LEG) return;
      if(mx>tp.x-ts.w/2-EDGE_MIN_LEG && mx<tp.x+ts.w/2+EDGE_MIN_LEG) return;
      const k=Math.round(mx);
      if(seen.has(k)) return;
      seen.add(k);
      if(seen.size>8) return;
      const a0=sidePt(sp.x,sp.y,ss.w,ss.h, mx>=sp.x?'R':'L');
      const a3=sidePt(tp.x,tp.y,ts.w,ts.h, mx>=tp.x?'R':'L');
      add([a0,{x:mx,y:a0.y},{x:mx,y:a3.y},a3], 'ortho2-h', 10+i);
    });
  }
  {
    const epT=Math.min(sp.y-ss.h/2, tp.y-ts.h/2)-EDGE_LANE;
    const epB=Math.max(sp.y+ss.h/2, tp.y+ts.h/2)+EDGE_LANE;
    const midYs=[epT, epB, (sp.y+tp.y)/2];
    for(const t of edgeObstacles){
      if(t===edge.source||t===edge.target) continue;
      const box=edgeExpandedBox(t);
      if(!box) continue;
      const lo=Math.min(sp.y,tp.y), hi=Math.max(sp.y,tp.y);
      if(box.y1<lo-EDGE_LANE||box.y0>hi+EDGE_LANE) continue;
      midYs.push(box.y0-EDGE_LANE, box.y1+EDGE_LANE);
    }
    const seen=new Set();
    midYs.forEach((my,i)=>{
      if(my>sp.y-ss.h/2-EDGE_MIN_LEG && my<sp.y+ss.h/2+EDGE_MIN_LEG) return;
      if(my>tp.y-ts.h/2-EDGE_MIN_LEG && my<tp.y+ts.h/2+EDGE_MIN_LEG) return;
      const k=Math.round(my);
      if(seen.has(k)) return;
      seen.add(k);
      if(seen.size>8) return;
      const a0=sidePt(sp.x,sp.y,ss.w,ss.h, my>=sp.y?'B':'T');
      const a3=sidePt(tp.x,tp.y,ts.w,ts.h, my>=tp.y?'B':'T');
      add([a0,{x:a0.x,y:my},{x:a3.x,y:my},a3], 'ortho2-v', 20+i);
    });
  }

  // Prefer clear routes; among those: fewer bends, shorter, more clearance, stable ord.
  // Always try the direct segment first conceptually — if it clears, it ranks first
  // (0 bends + typically shortest + ord 0).
  const clear=cands.filter(c=>!c.blocked);
  const pool=clear.length?clear:cands;
  pool.sort((a,b)=>{
    if(a.blocked!==b.blocked) return a.blocked?1:-1;
    if(a.bends!==b.bends) return a.bends-b.bends;
    if(Math.abs(a.len-b.len)>0.5) return a.len-b.len;
    if(Math.abs(a.clear-b.clear)>0.5) return b.clear-a.clear;
    return a.ord-b.ord;
  });
  const best=pool[0];

  // If every orth candidate is blocked, fall back to a bounded perpendicular
  // cubic (legacy-style) — never silently claim a blocked orth path is fine.
  if(!best||best.blocked){
    return routeEdgeBezierFallback(edge, sp, ss, tp, ts);
  }
  return {
    d: pathDFromPts(best.pts),
    pts: best.pts,
    kind: best.kind,
    // seed only — drawEdge runs throughLabelPos with the real label size
    thrSeed: longestSegmentMid(best.pts),
  };
}

// Legacy cubic used only when no clean straight/orthogonal route exists.
// Still tries increasing bends to reduce table crossings; never forces a
// bend solely because multiple associations are consolidated.
function routeEdgeBezierFallback(edge, sp, ss, tp, ts){
  const src=borderPt(sp.x,sp.y,ss.w,ss.h,tp.x,tp.y);
  const tgt=borderPt(tp.x,tp.y,ts.w,ts.h,sp.x,sp.y);
  const dx=tgt.x-src.x, dy=tgt.y-src.y;
  const dist=Math.sqrt(dx*dx+dy*dy)||1;
  const nx=-dy/dist, ny=dx/dist;
  let bend=0, bestN=Infinity, best=0;
  const bendCands=[0];
  for(let m=20; m<=240; m+=20){ bendCands.push(m,-m); }
  for(const b of bendCands){
    const c1={x:src.x+dx*CURVE_T+nx*b, y:src.y+dy*CURVE_T+ny*b};
    const c2={x:tgt.x-dx*CURVE_T+nx*b, y:tgt.y-dy*CURVE_T+ny*b};
    // sample for residual crossings
    let blocked=0;
    ...
    for(const t of edgeObstacles){
      ...
      const p=nodePos[t], s=nodeSize[t];
      if(!p||!s) continue;
      const x0=p.x-s.w/2-EDGE_CLEAR, x1=p.x+s.w/2+EDGE_CLEAR;
      const y0=p.y-s.h/2-EDGE_CLEAR, y1=p.y+s.h/2+EDGE_CLEAR;
    ...
    if(blocked===0){ bend=b; bestN=0; break; }
    if(blocked<bestN){ bestN=blocked; best=b; }
  }
  if(bestN>0) bend=best;
  ...
  // Curve midpoint seed only — evacuation needs the real thr pill size in drawEdge
  const thrSeed={x:(src.x+tgt.x)/2+nx*bend*.5, y:(src.y+tgt.y)/2+ny*bend*.5};
  return {
...
function drawEdge(parent, edge) {
  ...
  // self-referential association: compact loop (curve is appropriate here)
  if(edge.source===edge.target){
    const s=nodeSize[edge.source]||calcSize(edge.source);
  ...
  if(thr){
    const lw=thr.length*6+6, lh=14;
    // Always evacuate with the real pill size — including fallback-curve seeds
    // that would otherwise skip throughLabelPos via a pre-baked thrAt center.
    const seed=route.thrSeed||longestSegmentMid(route.pts);
    const mid=throughLabelPos(seed, lw, lh);
    g.appendChild(svgEl('rect',{x:mid.x-lw/2,y:mid.y-lh/2,width:lw,height:lh,rx:3,class:'e-lbg'}));
    const lt=svgEl('text',{x:mid.x,y:mid.y+1,'text-anchor':'middle','dominant-baseline':'middle',class:'e-ltxt'});
...
// Remove a table from the current Overview — the same effect as unchecking
...
//
// A *kept* (retained-after-auto-expand-OFF) table is already unchecked, so
// simply re-adding it to excludedTables (the old body of this function)
// was a no-op — it never actually left the screen. Drop it from
// retainedExpandedTables instead; only fall back to excludedTables.add for
// a table that's actually explicit right now.
function excludeTable(name){
  if(retainedExpandedTables.has(name)){ retainedExpandedTables.delete(name); }
...
// The mirror image of excludeTable(): promote a currently auto-expanded or
// kept (dashed, not-yet-explicit) table to an explicit root — exactly what
// checking its list checkbox does (see renderTableList()), and otherwise
// unreachable for an 'auto' table since its checkbox there is disabled.
...
function renderTableList(){
  ...
  list.innerHTML='';
  // Computed once for the whole list render (not per row) — overviewDisplayKind()
  // relies on this instead of calling getDisplayTables() itself, which would
  // otherwise make a full list render O(n²) in the table count.
  const displaySet = new Set(getDisplayTables());
  ...
  allTables()
    ...
    .forEach(name => {
      ...
      // shown because auto-expansion (or a kept-after-OFF leftover) pulled
      // it in, not by its own checkbox — while focused, every related table
      // other than the focus target itself counts the same way
      const kind = focusedTable
        ? (isHidden ? 'banned' : focusedTable===name ? 'focused'
           : displaySet.has(name) ? 'focus-related' : 'excluded')
        : overviewDisplayKind(name, displaySet);
      const autoShown = kind==='auto' || kind==='retained' || kind==='focus-related';
      ...
      // Overview auto-expand or kept-after-OFF (not focus mode): the
      // checkbox stays enabled and doubles as the promote-to-explicit-root
      // action, routed through the same promoteAuto() the node's own ＋
      // button uses (drawNode) — one shared function so the two never
      // disagree about what "promote" actually does. Focus mode's own
      // "every other table looks auto-shown" case is left locked, unchanged:
      // focus ignores checkboxes for visibility entirely, so unlocking
      // there would silently update excludedTables with no visible effect
      // until the user exits focus — more confusing than useful.
      ...
      cb.addEventListener('change', e => {
        e.stopPropagation();
        if(cb.checked){ promoteAuto(name); return; }
        excludedTables.add(name);
...
function renderGlobalNotes(){
  ...
  box.querySelectorAll('[data-note-edit]').forEach(a=>{
    a.addEventListener('click', e=>{
      e.stopPropagation();
      const nid=a.getAttribute('data-note-edit');
      const n=NOTES.find(item=>ensureNoteId(item)===nid);
  ...
  box.querySelectorAll('[data-note-delete]').forEach(a=>{
    a.addEventListener('click', e=>{
      e.stopPropagation();
      const nid=a.getAttribute('data-note-delete');
      deleteNote(nid);
...
// Displayed-table pairs that overlap right now and involve one of `moved` —
// the only pairs a reposition of `moved` can create or clear. Keys are the
// two names sorted and joined, so a before/after pair of sets can be diffed
// directly (and a moved-vs-moved pair recorded from both ends collapses).
function overlapPairsWith(moved){
  const out=new Set(), all=getDisplayTables();
  moved.forEach(a=>{
    const pa=nodePos[a]; if(!pa) return;
    const sa=nodeSize[a]||calcSize(a);
    all.forEach(b=>{
      if(a===b) return;
      const pb=nodePos[b]; if(!pb) return;
      const sb=nodeSize[b]||calcSize(b);
...
// Align/Distribute do exactly what they promise, and — as in every design
// tool — that can legitimately land tables on top of each other (aligning a
// row's left edges stacks it). So the operation is never refused for
// overlapping; unlike Distribute's impossible case, nothing about the
// contract is broken. But an ER node hides real content when covered and
// there is no z-order control to dig it back out, so an overlap this
// operation just created is named in the toast and flashed on the canvas
// instead of happening silently. Overlaps that were already there are not
// reported — they aren't news, and re-reporting them would train the user
// to ignore the message.
function reportRepositionOverlaps(msg, moved, before){
  const fresh=[...overlapPairsWith(moved)].filter(k=>!before.has(k));
  if(!fresh.length){ showToast(msg); return; }
  const names=new Set();
  fresh.forEach(k=>k.split('\u0000').forEach(n=>names.add(n)));
  names.forEach(flashNode);
  showToast(`${msg} — ${names.size} now overlap (Undo to revert)`);
}

function alignSelection(mode){
  const ts=selectedPositioned();
  if(ts.length<2) return;
  const overlapsBefore=overlapPairsWith(ts);
  pushUndoSnapshot();
  ...
  reportRepositionOverlaps(`Aligned ${ts.length} tables`, ts, overlapsBefore);
}
...
function distributeSelection(axis){
  ...
  // The promise (UI + manual) is that the two outermost tables stay put and
  // only the gaps between them are equalized. When the nodes are collectively
  // wider than that span there is no such arrangement: packing them anyway
  // would keep the near end fixed and shove the far one outward, silently
  // breaking the contract. Do nothing instead — no move, no undo entry.
  if(totalSize>totalSpan){ showToast('Not enough space to distribute without overlap'); return; }
  const gap=(totalSpan-totalSize)/(items.length-1);
  // equalizing the gaps can still slide a member onto an unselected table
  // off to the side, even though the selection itself now fits
  const overlapsBefore=overlapPairsWith(ts);
  pushUndoSnapshot();
  ...
  afterManualReposition();
  reportRepositionOverlaps(`Distributed ${ts.length} tables`, ts, overlapsBefore);
}
...
window.addEventListener('mousemove', e=>{
  if(isDragging&&dragName){
    ...
    // diagram's edges are untouched by this drag, and re-running the edge
...
document.getElementById('btn-reset')   .addEventListener('click',()=>{
  ...
  // Overview ↺ honors layoutOrientation once Horizontal/Auto engines land;
  // focus ↺ always keeps the hub-spoke Vertical path via preferredHub.
...
// Layout orientation: overview full re-layout on change; no-op while focused
// or when the chosen orientation is not implemented yet (disabled buttons).
document.querySelectorAll('.lo-btn').forEach(btn=>{
  btn.addEventListener('click',()=>{
    if(focusedTable) return;
    const next=normalizeLayoutOrientation(btn.dataset.lo);
    if(!LAYOUT_ORIENTATION_IMPLEMENTED[next]) return;
    if(next===layoutOrientation) return;
    layoutOrientation=next;
    saveState();
    updateLayoutOrientationUI();
    pushUndoSnapshot(); // positions only — layoutOrientation is not undone
...
document.getElementById('btn-none').addEventListener('click',()=>{
  ...
  retainedExpandedTables.clear(); noAutoExpandRoot.clear();
...
function updateFocusUI(){
  ...
  // Layout is overview-only — disable the control while focused so a click
  // cannot run an overview full-layout against the focus node set
  updateLayoutOrientationUI();
}

// Layout orientation toolbar (Auto / Vertical / Horizontal). Overview-only:
// disabled in focus mode. Unimplemented options (Auto/Horizontal until their
// engines land) stay disabled with "Coming later" tooltips so the control
// never looks like a working feature that silently re-packs as Vertical.
// Changing an *implemented* option pushes one undo of *positions* and runs
// exactly one full overview layout + one render + one fit (same shape as ↺),
// even when Auto-tidy is off. The selected policy itself is not part of undo.
function updateLayoutOrientationUI(){
  const focused=!!focusedTable;
  document.querySelectorAll('.lo-btn').forEach(b=>{
    const lo=b.dataset.lo;
    const impl=!!LAYOUT_ORIENTATION_IMPLEMENTED[lo];
    b.classList.toggle('active', lo===layoutOrientation);
    b.disabled=focused || !impl;
    if(lo==='auto'){
      b.title=impl
        ? 'Auto: pick Vertical or Horizontal for the current viewport on overview full layouts (near-ties keep Vertical)'
        : 'Auto: Coming later — will pick Vertical or Horizontal for the viewport';
    } else if(lo==='horizontal'){
      b.title=impl
        ? 'Horizontal: hub-centered left/right branches (overview full layouts only)'
        : 'Horizontal: Coming later — hub-centered left/right branches';
    } else {
      b.title='Vertical: top/bottom depth rows (compatibility default)';
    }
  });
  const g=document.getElementById('layout-orient-group');
  if(g){
    g.title=focused
      ? 'Layout applies to the overview only (disabled while focused). Use ↺ inside Focus for the hub-spoke layout.'
      : 'Layout orientation for the overview (Auto / Vertical / Horizontal). Independent of Direction (Both/Deps/Dependents). Applies to overview Auto-tidy and ↺ only.';
  }
}

// One overview full re-layout driven by the current layoutOrientation.
// Mirrors #btn-reset's single-layout contract (clear pos → gridLayout once →
// render once → fit once). Caller is responsible for pushUndoSnapshot().
function relayoutOverviewForOrientation(){
  const ts=getDisplayTables();
  ts.forEach(t=>delete nodePos[t]);
  Object.keys(ringDepth).forEach(k=>delete ringDepth[k]);
  ts.forEach(n=>{ nodeSize[n]=calcSize(n); });
  // preferredHub stays undefined in overview; focus never reaches here
  if(ts.length) gridLayout(ts, undefined, true);
  renderDiagram();
  requestAnimationFrame(fitView);
}
...
document.getElementById('auto-expand').addEventListener('change', e=>{
  ...
  // Both branches below are skipped entirely while focused: the Overview's
  // own retained/checked bookkeeping isn't the active view at that point
  // (getDisplayTables() while focused reads focusedTable/manualExpanded,
  // never autoExpand or retainedExpandedTables), so touching either set
  // here would be invisible until the user exits focus and could silently
  // wipe out — or fabricate from the FOCUS view's own unrelated display set
  // — Overview state the user never asked to change. The flag itself still
  // flips either way; only its Overview-side bookkeeping effects wait.
  if (!focusedTable) {
    if (!turningOn) {
      // ON -> OFF: freeze whatever auto-expand alone is currently showing
      // (still-unchecked tables pulled in as someone's BFS neighbor) into
      // retainedExpandedTables *before* flipping the flag — getDisplayTables()
      // below still reads the old (true) autoExpand value at this point.
      const shownNow = getDisplayTables();
      retainedExpandedTables = new Set(shownNow.filter(t => excludedTables.has(t)));
    } else {
      // OFF -> ON: auto-expand's own BFS recomputes the expanded set from
      // scratch every render, so a stale retained set would only cause
      // confusion (and — combined with noAutoExpandRoot not applying to it —
      // would let repeated OFF/ON cycles creep the display outward).
      retainedExpandedTables.clear();
    }
  }
  autoExpand=turningOn;
...
function snapshotView(){
  ...
  return {excl:[...excludedTables], hid:[...hiddenTables], ret:[...retainedExpandedTables],
          ae:autoExpand, dep:expandDepth, dir:expandDir, lo:layoutOrientation, cm:colMode, pos};
}

function applyView(v){
  excludedTables=new Set(v.excl||[]); hiddenTables=new Set(v.hid||[]);
  // always REPLACE — a view/share payload predating this field (or one an
  // older erdscope wrote) has no `ret` at all, and even a well-formed one
  // must never merge with whatever is currently retained, or applying a
  // saved view could silently resurrect stale, unrelated retained tables
  const retRaw = Array.isArray(v.ret) ? v.ret : [];
  retainedExpandedTables = new Set(retRaw.filter(t => DATA.tables[t]));
  autoExpand=!!v.ae; expandDepth=v.dep??1; expandDir=v.dir||'both';
  // restore Layout policy only — do not re-layout; saved `pos` wins until
  // the next explicit overview ↺ / Layout change / Auto-tidy full pack
  layoutOrientation=normalizeLayoutOrientation(v.lo);
  if(autoExpand) retainedExpandedTables.clear(); // meaningless (and never produced) while auto-expand is on
  for (const t of retainedExpandedTables) if (!excludedTables.has(t)) retainedExpandedTables.delete(t);
  colMode=v.cm??0; colOverride={}; manualExpanded.clear(); noAutoExpandRoot.clear();
...
class SchemaGrid {
  constructor(containerEl) {
    ...
    this.filterQuery = '';
    this.filterScope = 'table'; // 'all' | 'table' | 'column' — a hub table name under 'all' matches every table that references it, which reads as "no filtering happened"
    ...
    if (this.activeTab === 'tables') {
      ...
      if (!tableRows.length) {
        ...
        tableRows.forEach(r => {
          ...
          if (r.hasNotes) {
            notesHtml = `
              <div class="grid-note-cell" data-grid-scope="table" data-grid-table="${esc(r.name)}" data-grid-nid="${esc(nid)}">
                <div class="grid-note-content">
                  ${isDetailed
                    ? `<div class="grid-note-text" data-grid-act="edit" title="Click to edit inline">${esc(noteTextVal)}</div>`
                    : `<span class="grid-badge grid-badge-notes" data-grid-act="edit" title="${esc(noteTextVal)}">✓ Note</span>`
                  }
                  <button class="grid-note-edit-btn" data-grid-act="edit" title="Edit inline">✏️</button>
                  <button class="grid-note-edit-btn" data-grid-act="modal" title="Advanced edit in modal">⚙️</button>
                </div>
              </div>`;
          } else {
            notesHtml = `
              <div class="grid-note-cell" data-grid-scope="table" data-grid-table="${esc(r.name)}">
                <button class="grid-note-btn" data-grid-act="edit">+ Note</button>
              </div>`;
          }
          ...
          const propBadge = r.isProposed ? '<span class="badge-proposed">PROPOSED</span>' : '';

          html += `<tr>
            <td>${grpHtml}</td>
            <td><a class="grid-table-link" data-goto-table="${esc(r.name)}">${esc(r.name)}</a>${propBadge}</td>
      ...
      if (!columnRows.length) {
        ...
        columnRows.forEach(r => {
          ...
          const scope = isFk ? 'relation' : 'table';
          ...
          if (r.hasNotes) {
            notesHtml = `
              <div class="grid-note-cell" data-grid-scope="${scope}" data-grid-table="${esc(r.tableName)}" data-grid-column="${esc(r.name)}" data-grid-nid="${esc(nid)}">
                <div class="grid-note-content">
                  ${isDetailed
                    ? `<div class="grid-note-text" data-grid-act="edit" title="Click to edit inline">${esc(noteTextVal)}</div>`
                    : `<span class="grid-badge grid-badge-notes" data-grid-act="edit" title="${esc(noteTextVal)}">✓ Note</span>`
            ...
            notesHtml = `
              <div class="grid-note-cell" data-grid-scope="${scope}" data-grid-table="${esc(r.tableName)}" data-grid-column="${esc(r.name)}">
                <button class="grid-note-btn" data-grid-act="edit">+ Note</button>
              </div>`;
          }
          ...
          html += `<tr>
            ...
            <td style="font-weight:600">${esc(r.name)}${propBadge}</td>
            <td>${esc(r.logical_name || '-')}</td>
  ...
  bindNoteEvents() {
    if (!this.container) return;
    this.container.querySelectorAll('[data-grid-act]').forEach(el => {
      el.addEventListener('click', e => {
        e.stopPropagation();
        const cell = el.closest('.grid-note-cell');
        if (!cell) return;
        const act = el.getAttribute('data-grid-act');
        const scope = cell.getAttribute('data-grid-scope') || 'table';
        const table = cell.getAttribute('data-grid-table') || '';
        const column = cell.getAttribute('data-grid-column') || '';
        const nid = cell.getAttribute('data-grid-nid') || '';
        const existingNote = nid ? NOTES.find(n => ensureNoteId(n) === nid) : null;

        if (act === 'modal') {
          if (typeof openNoteModal === 'function') {
            openNoteModal(existingNote || { scope, table, source_table: table, foreign_key: column });
          } else if (window.opener && !window.opener.closed && typeof window.opener.openNoteModal === 'function') {
            window.opener.openNoteModal(existingNote || { scope, table, source_table: table, foreign_key: column });
          }
          return;
        }
        ...
        if (act === 'edit') {
          ...
          cell.innerHTML = `
            <div class="grid-inline-editor">
              ...
              <div class="grid-inline-actions">
                <button class="grid-inline-save" type="button">Save</button>
                <button class="grid-inline-cancel" type="button">Cancel</button>
                <button class="grid-inline-modal" type="button">Modal...</button>
              </div>
            </div>`;
          const textarea = cell.querySelector('.grid-inline-textarea');
          if (textarea) {
            textarea.focus();
          ...
          const doSave = () => {
            ...
            saveGridNote(scope, table, column, val, nid);
          };
          ...
          cell.querySelector('.grid-inline-save')?.addEventListener('click', ev => {
            ev.stopPropagation();
            doSave();
          });

          cell.querySelector('.grid-inline-cancel')?.addEventListener('click', ev => {
            ev.stopPropagation();
            doCancel();
          });

          cell.querySelector('.grid-inline-modal')?.addEventListener('click', ev => {
            ev.stopPropagation();
            if (typeof openNoteModal === 'function') {
              openNoteModal(existingNote || { scope, table, source_table: table, foreign_key: column });
            } else if (window.opener && !window.opener.closed && typeof window.opener.openNoteModal === 'function') {
...
function closeModalElement(modal) {
  if (!modal) return;
  const id = modal.id;
  if (id === 'schema-grid-modal') closeSchemaGridModal();
  else if (id === 'unsaved-config-modal') closeUnsavedConfigModal();
  else if (id === 'proposed-table-modal') closeProposedTableModal();
  else if (id === 'proposed-column-modal') closeProposedColumnModal();
  else if (id === 'note-edit-modal') closeNoteModal();
...
function savePersistedConfig() {
  try {
    const configStr = exportConfigJSON();
    setLS(LS('persisted_config'), configStr);
  } catch(e) {}
}

function restorePersistedConfigOnLoad() {
  try {
    const raw = localStorage.getItem(LS('persisted_config'));
    if (!raw) return;
    const cfg = JSON.parse(raw);
    if (cfg.notes && Array.isArray(cfg.notes)) {
      const restoredNotes = cfg.notes.map(n => {
        const item = { id: n.id, title: n.title, text: n.text, links: n.links };
        if (n.target) {
          item.scope = n.target.type || 'table';
          item.table = n.target.table || n.target.source_table || '';
          item.source_table = n.target.source_table || n.target.table || '';
          item.target_table = n.target.target_table || n.target.target || '';
          item.foreign_key = n.target.foreign_key;
          item.through = n.target.through;
        }
        return item;
      });
      NOTES.length = 0;
    ...
    if (cfg.groups && Array.isArray(cfg.groups)) {
      GROUPS.length = 0;
      GROUPS.push(...cfg.groups);
    }
    if (cfg.tables && typeof cfg.tables === 'object') {
      Object.keys(cfg.tables).forEach(tblName => {
        const tblCfg = cfg.tables[tblName];
        if (!tblCfg) return;
        let t = DATA.tables[tblName];
        if (!t) {
          t = {
            name: tblName,
            logical_name: tblCfg.logical_name || '',
            comment: tblCfg.comment || '',
            columns: [],
            associations: [],
            indexes: [],
            isProposed: true
          };
          DATA.tables[tblName] = t;
        }
        if (tblCfg.columns && Array.isArray(tblCfg.columns)) {
          tblCfg.columns.forEach(cCfg => {
            let col = (t.columns || []).find(c => c.name === cCfg.name);
            if (!col) {
              col = {
                name: cCfg.name,
                logical_name: cCfg.logical_name || '',
                type: cCfg.type || 'varchar(255)',
                nullable: !!cCfg.nullable,
                comment: cCfg.comment || '',
                primary: !!cCfg.primary,
                isProposed: true
              };
              t.columns.push(col);
            }
    ...
    console.warn('Could not restore persisted config:', e);
  }
}
...
function exportConfigJSON() {
  const configNotes = NOTES.map((n, idx) => {
    const entry = { id: n.id || ('note_' + (idx + 1)) };
    if (n.scope === 'table') {
      entry.target = { type: 'table', table: n.table || n.source_table || '' };
    } else if (n.scope === 'relation') {
      const tgt = {
        type: 'relation',
        source_table: n.source_table || n.table || '',
        target_table: n.target_table || n.target || ''
      };
      if ('foreign_key' in n) tgt.foreign_key = n.foreign_key;
      else if (n.foreign_key !== undefined) tgt.foreign_key = n.foreign_key;

      if ('through' in n) tgt.through = n.through;
      else if (n.through !== undefined) tgt.through = n.through;

      if (n.name) tgt.name = n.name;
      if (n.assoc_type || n.type) tgt.assoc_type = n.assoc_type || n.type;
      if (typeof n.polymorphic === 'boolean') tgt.polymorphic = n.polymorphic;
      entry.target = tgt;
    } else {
      entry.target = { type: 'global' };
    }
    if (n.title) entry.title = n.title;
    if (n.text) entry.text = n.text;
    if (n.links && n.links.length) entry.links = n.links;
    return entry;
  });

  const configTables = {};
  Object.keys(DATA.tables || {}).forEach(tblName => {
    const t = DATA.tables[tblName];
    if (!t) return;
    const proposedCols = (t.columns || []).filter(c => c.isProposed);
    if (t.isProposed || proposedCols.length > 0) {
      const tblConfig = {};
      if (t.logical_name) tblConfig.logical_name = t.logical_name;
      if (t.comment) tblConfig.comment = t.comment;
      tblConfig.columns = (t.columns || []).map(c => {
        const colObj = { name: c.name, type: c.type || 'varchar(255)' };
        if (c.logical_name) colObj.logical_name = c.logical_name;
        if (c.nullable) colObj.nullable = true;
        if (c.comment) colObj.comment = c.comment;
        if (c.primary) colObj.primary = true;
        return colObj;
      });
      configTables[tblName] = tblConfig;
    }
  });
...
function generateUpdatedHTMLSource() {
  ...
  try {
    // `;\r?\n` (not just `;\n`) — document.documentElement.outerHTML can carry
    // CRLF line endings if the page itself was served/saved with them, and a
    // bare `;\n` never matches inside a CRLF run (every \n is preceded by \r),
    // silently leaving the ORIGINAL DATA/NOTES/GROUPS in the exported file.
    htmlContent = htmlContent.replace(/(?:const|let|var) DATA = [\s\S]*?;\r?\n/m, `const DATA = ${safeJsonForScript(DATA)};\n`);
    htmlContent = htmlContent.replace(/(?:const|let|var) NOTES = [\s\S]*?;\r?\n/m, `const NOTES = ${safeJsonForScript(NOTES)};\n`);
    htmlContent = htmlContent.replace(/(?:const|let|var) GROUPS = [\s\S]*?;\r?\n/m, `const GROUPS = ${safeJsonForScript(GROUPS)};\n`);
...
function updateNoteFormFields() {
  ...
  const grpSrc = document.getElementById('note-target-source-group');
  const grpRelTgt = document.getElementById('note-target-rel-table-group');
  const grpFk = document.getElementById('note-target-fk-group');

  if (scope === 'table') {
    if (grpTable) grpTable.style.display = '';
    if (grpSrc) grpSrc.style.display = 'none';
    if (grpRelTgt) grpRelTgt.style.display = 'none';
    if (grpFk) grpFk.style.display = 'none';
  } else if (scope === 'relation') {
    if (grpTable) grpTable.style.display = 'none';
    if (grpSrc) grpSrc.style.display = '';
    if (grpRelTgt) grpRelTgt.style.display = '';
    if (grpFk) grpFk.style.display = '';
  } else {
    if (grpTable) grpTable.style.display = 'none';
    if (grpSrc) grpSrc.style.display = 'none';
    if (grpRelTgt) grpRelTgt.style.display = 'none';
...
document.getElementById('note-edit-scope')?.addEventListener('change', updateNoteFormFields);
...
function openNoteModal(n = {}) {
  ...
  document.getElementById('note-edit-table').value = n.table || '';
  document.getElementById('note-edit-source-table').value = n.source_table || n.table || '';
  document.getElementById('note-edit-target-table').value = n.target_table || n.target || '';
  document.getElementById('note-edit-foreign-key').value = n.foreign_key || '';

  document.getElementById('note-edit-title-val').value = n.title || '';
  document.getElementById('note-edit-text-val').value = n.text || '';
  updateNoteFormFields();
}
...
function saveNoteFromModal() {
  ...
  const sourceTableVal = document.getElementById('note-edit-source-table').value.trim();
  const targetTableVal = document.getElementById('note-edit-target-table').value.trim();
  const fkVal = document.getElementById('note-edit-foreign-key').value.trim();
  ...
  if (scope === 'table') {
    ...
    delete note.target;
    delete note.target_table;
    delete note.foreign_key;
  } else if (scope === 'relation') {
    note.source_table = sourceTableVal;
    note.target = targetTableVal;
    note.target_table = targetTableVal;
    if (fkVal) note.foreign_key = fkVal; else delete note.foreign_key;
    delete note.table;
  } else {
    delete note.table;
    delete note.source_table;
...
function saveGridNote(scope, table, column, text, nid = null) {
  text = (text || '').trim();
  let note = nid ? NOTES.find(n => ensureNoteId(n) === nid) : null;
  if (!note) {
    if (scope === 'table') {
      note = NOTES.find(n => n.scope === 'table' && n.table === table);
    } else if (scope === 'relation') {
      note = NOTES.find(n => n.scope === 'relation' && n.source_table === table && n.foreign_key === column);
    }
  }

  if (!text) {
    if (note) {
      deleteNote(ensureNoteId(note));
    } else {
      if (typeof refreshAllNoteViews === 'function') refreshAllNoteViews();
    }
    return;
  }
  ...
  note.text = text;
  markConfigDirty();
  if (typeof showToast === 'function') showToast('Saved note ✓');
  if (typeof refreshAllNoteViews === 'function') refreshAllNoteViews();
}
...
// ── Proposed ToBe Schema & Unsaved Config Handlers ───────────────────────
function addProposedTable(name, logical, comment, group) {
  name = (name || '').trim();
  if (!name) { showToast('Table name is required'); return false; }
  if (Object.hasOwn(DATA.tables || {}, name)) { showToast(`Table "${name}" already exists`); return false; }
  ...
  DATA.tables[name] = {
    ...
    comment: comment || '',
    columns: [
      { name: 'id', type: 'bigint', primary: true, nullable: false, comment: 'Primary Key', isProposed: true }
    ],
    associations: [],
    indexes: [],
    isProposed: true
  ...
  if (group) {
    let g = GROUPS.find(item => item.id === group || item.title === group);
    if (!g) {
      g = { id: group.toLowerCase().replace(/\s+/g, '_'), title: group, tables: [] };
      GROUPS.push(g);
    }
  ...
  markConfigDirty();
  showToast(`Added proposed table "${name}" ✓`);
  refreshView();
...
function addProposedColumn(tableName, colName, logical, colType, nullableVal, comment) {
  tableName = (tableName || '').trim();
  colName = (colName || '').trim();
  if (!tableName || !Object.hasOwn(DATA.tables || {}, tableName)) {
    showToast(`Unknown target table: "${tableName}"`);
    return false;
  }
  if (!colName) { showToast('Column name is required'); return false; }

  const tbl = DATA.tables[tableName];
  tbl.columns = tbl.columns || [];
  if (tbl.columns.some(c => c.name === colName)) {
    showToast(`Column "${colName}" already exists in table "${tableName}"`);
    return false;
  }
  ...
  tbl.columns.push({
    ...
    nullable: nullableVal === 'YES',
    comment: comment || '',
    isProposed: true
  });

  markConfigDirty();
  showToast(`Added proposed column "${colName}" to "${tableName}" ✓`);
  refreshView();
  if (schemaGrid) schemaGrid.render();
  return true;
}

function openProposedTableModal() {
  const m = document.getElementById('proposed-table-modal');
  if (m) m.classList.remove('hidden');
  document.getElementById('prop-table-name').value = '';
  document.getElementById('prop-table-logical').value = '';
  document.getElementById('prop-table-comment').value = '';
  document.getElementById('prop-table-group').value = '';
}
function closeProposedTableModal() {
  const m = document.getElementById('proposed-table-modal');
  if (m) m.classList.add('hidden');
}

function openProposedColumnModal() {
  const m = document.getElementById('proposed-column-modal');
  if (!m) return;
  const sel = document.getElementById('prop-col-table-sel');
  if (sel) {
    sel.innerHTML = Object.keys(DATA.tables || {}).sort().map(t => `<option value="${esc(t)}">${esc(t)}</option>`).join('');
  }
  m.classList.remove('hidden');
  document.getElementById('prop-col-name').value = '';
  document.getElementById('prop-col-logical').value = '';
  document.getElementById('prop-col-type').value = 'varchar(255)';
  document.getElementById('prop-col-comment').value = '';
}
function closeProposedColumnModal() {
  const m = document.getElementById('proposed-column-modal');
  if (m) m.classList.add('hidden');
}

function openUnsavedConfigModal() {
  const m = document.getElementById('unsaved-config-modal');
  if (m) m.classList.remove('hidden');
}
function closeUnsavedConfigModal() {
  const m = document.getElementById('unsaved-config-modal');
  if (m) m.classList.add('hidden');
}

document.getElementById('btn-grid-add-table')?.addEventListener('click', openProposedTableModal);
document.getElementById('btn-grid-add-col')?.addEventListener('click', openProposedColumnModal);

document.getElementById('btn-prop-table-close')?.addEventListener('click', closeProposedTableModal);
document.getElementById('btn-prop-table-cancel')?.addEventListener('click', closeProposedTableModal);
document.getElementById('btn-prop-table-save')?.addEventListener('click', () => {
  const name = document.getElementById('prop-table-name').value;
  const logical = document.getElementById('prop-table-logical').value;
  const comment = document.getElementById('prop-table-comment').value;
  const group = document.getElementById('prop-table-group').value;
  if (addProposedTable(name, logical, comment, group)) {
    closeProposedTableModal();
  }
});

document.getElementById('btn-prop-col-close')?.addEventListener('click', closeProposedColumnModal);
document.getElementById('btn-prop-col-cancel')?.addEventListener('click', closeProposedColumnModal);
document.getElementById('btn-prop-col-save')?.addEventListener('click', () => {
  const table = document.getElementById('prop-col-table-sel').value;
  const name = document.getElementById('prop-col-name').value;
  const logical = document.getElementById('prop-col-logical').value;
  const type = document.getElementById('prop-col-type').value;
  const nullable = document.getElementById('prop-col-nullable').value;
  const comment = document.getElementById('prop-col-comment').value;
  if (addProposedColumn(table, name, logical, type, nullable, comment)) {
    closeProposedColumnModal();
  }
});

document.getElementById('config-dirty-badge')?.addEventListener('click', openUnsavedConfigModal);
...
document.getElementById('btn-unsaved-copy')?.addEventListener('click', () => {
  document.getElementById('btn-export-config-copy')?.click();
  closeUnsavedConfigModal();
});
