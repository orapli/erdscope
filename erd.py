...
"""
import ast, os, subprocess, sys, json, re, argparse
...
body.dark .col-entry,body.dark .assoc-entry,body.dark .idx-entry{background:#1e293b}
body.dark .idx-name{color:#e2e8f0}
body.dark .col-cn,body.dark .aname{color:#e2e8f0}
    ...
    <div id="legend">
      ...
      <div id="legend-body">
        ...
        <div class="lhint">Framework association names (has_many etc.) appear in the right pane<br>
          Diagram: click = select, double-click = focus, drag = move<br>
...
function layoutAll(tables, edges) {
  ...
  if (newTables.length === tables.length) {
    ...
    // overview: compact aspect-fitted grid — readable at fit zoom,
    // no force sim (it inflates the layout into an unreadable circle)
    gridLayout(tables);
    return;
  }
...
function drawNode(parent, name) {
  ...
  const g = svgEl('g', {
    class: 'er-node' + (name===selectedTable?' sel':'') + (name===focusedTable?' center':'') + (isAuto?' auto':'') + ringCls,
...
// ── Select / focus ────────────────────────────────────────────────────────
// selectOnly: highlight + details, no relayout (diagram click). null clears.
function selectOnly(name){
  selectedTable=name||null;
  document.querySelectorAll('.er-node').forEach(el =>
    el.classList.toggle('sel', !!name && el.getAttribute('data-name')===name));
  updateEdgeHighlight();
  document.querySelectorAll('#table-list .table-item').forEach(el=>{
    const nm=el.querySelector('.tname')?.textContent;
    el.classList.toggle('selected', !!name && nm===name && !el.classList.contains('focused'));
  });
  showDetails(name);
}

// color the edges (and their end markers) that touch the selected table
function updateEdgeHighlight(){
  document.querySelectorAll('.er-edge').forEach(el=>{
    const on = !!selectedTable &&
      (el.getAttribute('data-source')===selectedTable || el.getAttribute('data-target')===selectedTable);
...
function clearFocus(){
  ...
  focusedTable=null; selectedTable=null;
  manualExpanded.clear();
  exitFocusMode();
  refreshView();
  showDetails(null); renderTableList();
  updateDepthCtrl(); updateFocusUI();
  ...
  allTables()
    ...
    .forEach(name => {
      ...
      item.className='table-item'
        ...
        +(selectedTable===name&&focusedTable!==name?' selected':'')
        +(isHidden?' hidden':'')
        +(autoShown?' inview':'');
      ...
      hb.addEventListener('click', e => {
        ...
        if(isHidden){ hiddenTables.delete(name); }
          ...
          if(focusedTable===name){ focusedTable=null; selectedTable=null; exitFocusMode(); }
        }
        saveState();
...
window.addEventListener('mousemove', e=>{
  if(isDragging&&dragName){
    ...
    const pt=svgPt(e.clientX,e.clientY);
    const nx=pt.x-dragOX, ny=pt.y-dragOY;
    nodePos[dragName]={x:nx,y:ny}; dragMoved=true;
    document.querySelectorAll(`.er-node[data-name="${CSS.escape(dragName)}"]`).forEach(el=>{
      const sz=nodeSize[dragName]||{w:160,h:100};
      el.setAttribute('transform',`translate(${nx-sz.w/2},${ny-sz.h/2})`);
    });
...
window.addEventListener('keydown', e=>{
  ...
  if(selectedTable) selectOnly(null);
});
...
document.getElementById('btn-reset')   .addEventListener('click',()=>{
  ...
  ts.forEach(t=>delete nodePos[t]);
  Object.keys(ringDepth).forEach(k=>delete ringDepth[k]);
  // overview: compact aspect-fitted grid (no force sim) = largest nodes on screen;
...
document.getElementById('btn-none').addEventListener('click',()=>{
  ...
  selectedTable=null;
  saveState(); refreshView(); renderTableList(); showDetails(null);
});
...
document.getElementById('max-rows').addEventListener('change', e=>{
  ...
  Object.keys(nodeSize).forEach(k=>delete nodeSize[k]);
  // node heights change drastically — re-layout to keep the no-overlap guarantee
...
function applyView(v){
  ...
  if(focusedTable){ focusedTable=null; selectedTable=null; exitFocusMode(); updateFocusUI(); }
  Object.keys(nodePos).forEach(k=>delete nodePos[k]);
  Object.keys(basePos).forEach(k=>delete basePos[k]);
  Object.keys(nodeSize).forEach(k=>delete nodeSize[k]);
  Object.keys(ringDepth).forEach(k=>delete ringDepth[k]);
  ...
  renderDiagram(); requestAnimationFrame(fitView);
  renderTableList(); updateHiddenBar(); updateDepthCtrl(); showDetails(null);
}
