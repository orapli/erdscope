...
// it from (Sol review). Members are never moved by either pass (see the
// memberSet skip below), so every frame's own bbox stays fixed while this
// is checked — only non-member positions need re-verifying.
...
//
// Never moves a group MEMBER (mirrors resolveGroupObstacles' own
// guarantee — see its "never moves a member" test): moving a member would
// shift that group's own frame bbox, which could newly enclose some other,
// previously-fine non-member table — a problem this pass has no way to
// even notice, let alone fix (Sol review). A member overlapping something
// is left for whichever OTHER table in the pair is not a member to move
// out of the way instead; the base gridLayout algorithm already guarantees
// members don't overlap each other from placement alone, so this never
// leaves a member-vs-member conflict unresolved.
...
// just the still-overlapping non-member tables to a deterministic column
// beyond the whole layout's bbox. Guaranteed overlap-free by construction
// (strictly beyond every other table's right edge, stacked with normal
// gaps), even though it may look visually odd — "no node-on-node overlap"
// is a harder requirement than "ideally placed", matching the review's ask
// for a safe fallback that prioritizes overlap-freedom. Never moves a
// group member, for the same reason resolveResidualOverlaps doesn't.
  ...
  for(let i=0;i<tables.length;i++){
    ...
    for(let j=i+1;j<tables.length;j++){
      ...
      if(Math.abs(pa.x-pb.x)<(sa.w+sb.w)/2 && Math.abs(pa.y-pb.y)<(sa.h+sb.h)/2){
        offenders.add(a); offenders.add(b);
      }
    }
  }
  if(!offenders.size) return;
  const memberSet=new Set(GROUPS.flatMap(g=>g.tables));
  const movable=[...offenders].filter(t=>!memberSet.has(t)).sort();
