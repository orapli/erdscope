...
// Place through-label; nudge off table interiors when the natural mid sits inside one.
function throughLabelPos(pts){
  ...
  for(const t of edgeObstacles){
    ...
    if(mid.x>x0&&mid.x<x1&&mid.y>y0&&mid.y<y1){
      // push to nearest edge of that table + small pad
      const dl=mid.x-x0, dr=x1-mid.x, dt=mid.y-y0, db=y1-mid.y;
      const m=Math.min(dl,dr,dt,db);
      if(m===dl) mid={x:x0-6, y:mid.y};
      else if(m===dr) mid={x:x1+6, y:mid.y};
      else if(m===dt) mid={x:mid.x, y:y0-6};
      else mid={x:mid.x, y:y1+6};
    }
  }
  return mid;
}
...
function routeEdge(edge){
  ...
  // 3. two-bend doglegs: H-first / V-first with a small fixed set of lanes
  {
    ...
    const a0=sidePt(sp.x,sp.y,ss.w,ss.h, signX>0?'R':'L');
    const a3=sidePt(tp.x,tp.y,ts.w,ts.h, signX>0?'L':'R');
    const midXs=[(a0.x+a3.x)/2];
    // lanes just outside intervening expanded boxes
    for(const t of edgeObstacles){
      if(t===edge.source||t===edge.target) continue;
      const box=edgeExpandedBox(t);
      if(!box) continue;
      // only consider obstacles roughly between the endpoints on X
      const lo=Math.min(a0.x,a3.x), hi=Math.max(a0.x,a3.x);
    ...
    midXs.forEach((mx,i)=>{
      ...
      seen.add(k);
      if(seen.size>6) return; // hard cap on candidate fan-out
      add([a0,{x:mx,y:a0.y},{x:mx,y:a3.y},a3], 'ortho2-h', 10+i);
    });
  }
  {
    const signY=tp.y>=sp.y?1:-1;
    const a0=sidePt(sp.x,sp.y,ss.w,ss.h, signY>0?'B':'T');
    const a3=sidePt(tp.x,tp.y,ts.w,ts.h, signY>0?'T':'B');
    const midYs=[(a0.y+a3.y)/2];
    for(const t of edgeObstacles){
      if(t===edge.source||t===edge.target) continue;
      const box=edgeExpandedBox(t);
      if(!box) continue;
      const lo=Math.min(a0.y,a3.y), hi=Math.max(a0.y,a3.y);
  ...
  return {
    ...
    pts: best.pts,
    kind: best.kind,
    thrAt: throughLabelPos(best.pts),
  };
}
...
function routeEdgeBezierFallback(edge, sp, ss, tp, ts){
  ...
  const thrAt={x:(src.x+tgt.x)/2+nx*bend*.5, y:(src.y+tgt.y)/2+ny*bend*.5};
  return {
...
function drawEdge(parent, edge) {
  ...
  if(thr){
    const mid=route.thrAt||longestSegmentMid(route.pts);
    const lw=thr.length*6+6;
    g.appendChild(svgEl('rect',{x:mid.x-lw/2,y:mid.y-8,width:lw,height:14,rx:3,class:'e-lbg'}));
