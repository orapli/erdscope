...
//
// Prefers evacuating only non-members, same as resolveResidualOverlaps —
// but if EVERY offender is a group member (Sol review, 5th pass: two
// DIFFERENT groups' members conflicting, with no non-member anywhere in
// the tangle), evacuating nothing would leave the overlap unresolved
// forever. At that point some member has to move regardless of whose
// frame it shifts — still strictly better than a permanent visible overlap.
function evacuateOverlappingTables(tables){
  ...
  const memberSet=new Set(GROUPS.flatMap(g=>g.tables));
  let movable=[...offenders].filter(t=>!memberSet.has(t)).sort();
  if(!movable.length) movable=[...offenders].sort(); // no non-member alternative anywhere in the tangle
