  ...
  <div id="center-pane">
    ...
    <div id="diagram-toolbar">
      ...
      <div class="tb-group">
        ...
        <div id="layout-orient-group" title="Layout orientation for the overview (Vertical / Horizontal; Auto coming later). Independent of Direction (Both/Deps/Dependents). Overview Auto-tidy and ↺ only — not Focus mode.">
          <span class="lo-label">Layout</span>
          <button type="button" class="diag-btn lo-btn" data-lo="auto" disabled title="Auto: Coming later — will pick Vertical or Horizontal for the viewport">Auto</button>
...
//
// Placement engines: Vertical and Horizontal are implemented. Auto still
// waits for candidate scoring (A3) and stays disabled with Coming later.
// Unimplemented options keep round-tripping in state/views/share so enabling
// them later does not break old payloads.
let layoutOrientation = 'vertical'; // 'auto' | 'vertical' | 'horizontal'
const LAYOUT_ORIENTATION_IMPLEMENTED = { auto: false, vertical: true, horizontal: true };
...
function layoutAllowsUpClearance(){
  // overview Horizontal only — focus always uses the Vertical engine
  return layoutOrientation==='horizontal' && !focusedTable;
}
...
function gridLayout(tables, preferredHub, adaptive) {
  ...
  // Auto still falls through to Vertical until A3 candidate scoring lands.
  const useHorizontal = !preferredHub && layoutOrientation==='horizontal';
  ...
  function runPass(policy){
    ...
    const boxes=comps.map(c=> useHorizontal ? layoutComponentHorizontal(c) : layoutComponent(c));
  ...
  // Vertical: adaptive evaluates row-width policies [1, 1.6, 'max'].
  // Horizontal has no width-policy variants yet — a single deterministic pass.
  const policies = useHorizontal ? [1] : (adaptive ? [1, 1.6, 'max'] : [1]);
  const candidates=policies.map(runPass);
...
function updateLayoutOrientationUI(){
  ...
  document.querySelectorAll('.lo-btn').forEach(b=>{
    ...
    if(lo==='auto'){
      b.title=impl
        ? 'Auto: pick Vertical or Horizontal for the current viewport (overview full layouts only)'
  ...
  if(g){
    g.title=focused
      ...
      : 'Layout orientation for the overview (Vertical / Horizontal; Auto coming later). Independent of Direction (Both/Deps/Dependents).';
  }
}
