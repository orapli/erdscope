  ...
  <div id="center-pane">
    ...
    <div id="diagram-toolbar">
      ...
      <div class="tb-group">
        ...
        <div id="layout-orient-group" title="Layout orientation for the overview. Only Vertical is active today; Auto and Horizontal are reserved and disabled until their placement engines land. Independent of Direction (Both/Deps/Dependents). Overview Auto-tidy and ↺ only — not Focus mode.">
          ...
          <button type="button" class="diag-btn lo-btn" data-lo="horizontal" disabled title="Horizontal: Coming later — hub-centered left/right branches">↔ Horizontal</button>
        </div>
...
//
// Placement engines: only Vertical is implemented today. Auto/Horizontal
// state still normalizes and round-trips so named views/share stay forward-
// compatible, but their toolbar buttons stay disabled (Coming later) until
// the engines land — do not re-enable the UI before then.
let layoutOrientation = 'vertical'; // 'auto' | 'vertical' | 'horizontal'
const LAYOUT_ORIENTATION_IMPLEMENTED = { auto: false, vertical: true, horizontal: false };
...
function updateLayoutOrientationUI(){
  ...
  if(g){
    g.title=focused
      ...
      : 'Layout orientation for the overview. Only Vertical is active today; Auto and Horizontal stay disabled until their placement engines land. Independent of Direction (Both/Deps/Dependents).';
  }
}
