...
function gridLayout(tables, preferredHub) {
  ...
  // it, and the MAX_ROW_W ceiling for wide/shallow fan-outs.
  const candidates=[1, 1.6, 'max'].map(runPass);
...
function layoutAll(tables, edges) {
  ...
  if(placedIsolated.length){
    let cix0=Infinity, ciy0=Infinity, cix1=-Infinity, ciy1=-Infinity;
    placedIsolated.forEach(t=>{
      const p=nodePos[t], s=nodeSize[t]||calcSize(t);
      cix0=Math.min(cix0, p.x-s.w/2); ciy0=Math.min(ciy0, p.y-s.h/2);
      cix1=Math.max(cix1, p.x+s.w/2); ciy1=Math.max(ciy1, p.y+s.h/2);
    });
    // bounded wrap: once this isolated column has grown well past the rest
    // of the diagram's height, start a fresh column to its right instead of
    // continuing to stack straight down — existing nodes are never moved
    // (Auto-tidy OFF's incremental path must leave prior placements alone),
    // this only changes where the *next* addition lands. Measured against
    // the CONNECTED (edged) tables' own bbox, not the whole display set's —
    // the whole set always includes this very column, which would make it
    // impossible for the column to ever out-grow "the rest of the diagram".
    ...
    if(ciy1-ciy0 > diagramH*1.5){ ix=cix1+40; iy=ciy0; }
    else { ix=cix0; iy=ciy1+40; }
  } else {
...
function refreshView(forceFit, forceRelayout){
  ...
  if(!focusedTable && autoLayout){
    ...
    const dimsInvalidated = ts.some(t=>!nodePos[t]); // e.g. colMode/nameMode/max-rows already cleared it
...
document.getElementById('btn-reset')   .addEventListener('click',()=>{
  ...
  ts.forEach(t=>delete nodePos[t]);
  Object.keys(ringDepth).forEach(k=>delete ringDepth[k]);
  // Every displayed table just lost its nodePos entry above, so
  // renderDiagram()'s layoutAll() sees the whole display set as "new" and
  // performs exactly one full gridLayout pass on its own (overview branch
  // when unfocused, hub-spoke branch with focusedTable as the preferred hub
  // otherwise) — calling gridLayout() here too, then going through
  // refreshView()'s Auto-tidy branch, used to lay the same tables out twice.
  renderDiagram();
