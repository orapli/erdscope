  ...
  <div id="center-pane">
    ...
    <div id="diagram-toolbar">
      ...
      <div class="tb-group">
        ...
        <div id="layout-orient-group" title="Layout orientation for the overview (Auto / Vertical / Horizontal). Independent of Direction (Both/Deps/Dependents), which controls relation traversal. Applies to overview Auto-tidy and ↺ only — not Focus mode.">
          <span class="lo-label">Layout</span>
          <button type="button" class="diag-btn lo-btn" data-lo="auto" title="Auto: pick Vertical or Horizontal for the current viewport (overview full layouts only)">Auto</button>
...
// Horizontal placement and Auto candidate scoring land in later commits;
// until then every orientation still runs the Vertical engine so the
// control's state, relayout, and persistence contracts are already real.
...
// (buttons are disabled). Horizontal/Auto placement engines come later —
// selecting them still runs one Vertical-engine pack so state/UI/relayout
// contracts are already real and testable.
...
// disabled in focus mode. Changing it pushes one undo of *positions* and
// runs exactly one full overview layout + one render + one fit (same shape
// as ↺), even when Auto-tidy is off. The selected policy itself is not part
// of undo — Undo restores coordinates but keeps layoutOrientation.
function updateLayoutOrientationUI(){
  const focused=!!focusedTable;
  document.querySelectorAll('.lo-btn').forEach(b=>{
    b.classList.toggle('active', b.dataset.lo===layoutOrientation);
    b.disabled=focused;
  });
